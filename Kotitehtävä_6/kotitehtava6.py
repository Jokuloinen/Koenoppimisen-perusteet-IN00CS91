...
for rounds in range(100):
    ...
    for i in range(M):
        ...
        grad_t0 = grad_t0 + ((t2 * pow(x[i], 2) + t1*x[i] + t0) - y[i]) * 1
        grad_t1 = grad_t1 + ((t2 * pow(x[i], 2) + t1*x[i] + t0) - y[i]) * x[i]
        grad_t2 = grad_t2 + ((t2 * pow(x[i], 2) + t1*x[i] + t0) - y[i]) * pow(x[i], 2)
